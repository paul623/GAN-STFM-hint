...
class SuperResTransformer(nn.Module):
    def __init__(self, *, dim, depth, heads, mlp_dim, dropout=0.1):
        super().__init__()
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))  # 一个可学习的参数，代表 Transformer 的类别标记，用于捕捉整体信息。
    ...
    def forward(self, img, mask=None, maps=None,  K=64):
        ...
        x = self.to_cls_token(x[:, 0])  # 将处理后的特征向量中的第一个位置的信息提取出来，作为最终的输出
...
class Encoder(nn.Module):
    def __init__(self, in_channels, num_layers, kernel_size, channel_size):
        '''
        Args:
...
class TRNet(nn.Module):
    ''' TRNet, a neural network for multi-frame super resolution (MFSR) by recursive fusion. 递归融合'''
    ...
    def forward(self, lrs, alphas, maps, K):
        ...
        ####################### fuse ######################
        img = layer1.permute(0, 3, 4, 1, 2).reshape(-1, layer1.shape[1],
                                                    layer1.shape[2])  # .contiguous().view == .reshape()
        if maps is not None:
