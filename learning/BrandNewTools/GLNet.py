    ...
    def collect_local_fm(self, image_global, patches, ratio, top_lefts, oped, batch_size, global_model=None,
        ...
        with torch.no_grad():
            if self.patch_n == 0:
                ...
                self.output_g, self.ps0_g, self.ps1_g, self.ps2_g, self.ps3_g = global_model.module.fpn_global.forward(
        ...
        if mode == 1:
            ...
            with torch.no_grad():
                if self.patch_n == 0:
                    ...
                    self.output_g, self.ps0_g, self.ps1_g, self.ps2_g, self.ps3_g = self.fpn_global.forward(self.c2_g,
                                                                                                            self.c3_g,
                                                                                                            self.c4_g,
                                                                                                            self.c5_g)
            ...
            output_g, ps0_g, ps1_g, ps2_g, ps3_g = self.fpn_global.forward(c2_g, c3_g, c4_g, c5_g, c2_ext=self.c2_l,
                                                                           c3_ext=self.c3_l, c4_ext=self.c4_l,
                                                                           c5_ext=self.c5_l, ps0_ext=self.ps0_l,
                                                                           ps1_ext=self.ps1_l, ps2_ext=self.ps2_l)
            # imsize = image_global.size()[2:]
            # output_g = F.interpolate(output_g, imsize, mode='nearest')
            self.clear_cache()
